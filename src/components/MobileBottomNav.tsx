...
import { Edit3, Palette, Eye } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useHaptics } from '@/hooks/useHaptics';

export type MobileTab = 'write' | 'style' | 'preview';
...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ activeTab, onTabChange }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden">
      <div className="bg-background/95 backdrop-blur-xl border-t border-border/50 px-2 pb-[env(safe-area-inset-bottom)]">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <motion.button
                ...
                className={cn(
                  "flex flex-col items-center gap-1 px-6 py-2 rounded-2xl transition-colors duration-200 relative",
