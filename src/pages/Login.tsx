...
import { PenTool, Mail, Lock, ArrowRight, Sparkles, Github } from 'lucide-react';
...
export default function Login() {
  ...
  const handleGoogleSignIn = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Google sign-in fail ho gaya. Email/Password ya GitHub try karo!');
    } finally {
      ...
      >
        ...
        >
          ...
          <div className="mt-4 p-3 rounded-xl bg-amber-50 border border-amber-200">
            <p className="text-[11px] text-amber-800 font-medium mb-1">⚠️ Google/GitHub OAuth Setup</p>
            <p className="text-[10px] text-amber-700 leading-relaxed">
              Agar Google ya GitHub sign-in kaam nahi kar raha, toh admin ko Supabase Dashboard mein jaake configure karna hoga:
              <br />→ Authentication → Providers → Google/GitHub → Client ID + Secret add karo
              <br />→ Redirect URL: <code className="bg-amber-100 px-1 rounded">https://ievggapvfidhygkhtkug.supabase.co/auth/v1/callback</code>
            </p>
          </div>
        </motion.div>
      </motion.div>
